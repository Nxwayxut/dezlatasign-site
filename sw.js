...
self.addEventListener("message", (event) => {
  if (event.data?.type === "SHOW_REMINDER") {
    ...
    event.waitUntil(self.registration.showNotification(title, { body, tag, icon: asset("favicon.svg"), badge: asset("favicon.svg"), vibrate: [120, 80, 120] }));
  }
});
self.addEventListener("push", (event) => {
  const data = event.data?.json() || {};
  event.waitUntil(self.registration.showNotification(data.title || "Не забывай", {
    body: data.body || "Пора немного позаботиться о себе", tag: data.tag || "self-care", icon: asset("favicon.svg"), badge: asset("favicon.svg"),
  }));
});
